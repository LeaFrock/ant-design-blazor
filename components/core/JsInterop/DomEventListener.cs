...
public class DomEventListener(IJSRuntime jsRuntime, DomEventSubscriptionStore domEventSubscriptionStore) : IDomEventListener
{
    ...
    public virtual void AddShared<T>(object dom, string eventName, Action<T> callback, bool preventDefault = false)
    {
        ...
        {
            value = [];
            domEventSubscriptionStore[key] = value;
        ...
        value.Add(new DomEventSubscription(callback, typeof(T), _id));
    }
    ...
    public void RemoveShared<T>(object dom, string eventName, Action<T> callback)
    {
        ...
        if (domEventSubscriptionStore.TryGetValue(key, out var value))
        {
            var subscription = value.Find(s => s.Delegate == (Delegate)callback);
            if (subscription != null && subscription.Id == _id)
            {
                domEventSubscriptionStore[key].Remove(subscription);
            }
        }
    }
    ...
    public void DisposeShared()
    {
        ...
        while (find)
        {
            ...
            if (key != null && subscription != null)
            {
                ...
                if (domEventSubscriptionStore[key].Count == 0 && _sharedDotNetObjectStore.TryGetValue(key, out var dotNetObject))
                {
                    jsRuntime.InvokeVoidAsync(JSInteropConstants.RemoveDomEventListener, key.Selector, key.EventName, dotNetObject);
    ...
    public async ValueTask AddResizeObserver(ElementReference dom, Action<List<ResizeObserverEntry>> callback)
    {
        ...
        {
            AddShared<JsonElement>("window", "resize", Call, false);

            void Call(JsonElement je) => callback.Invoke([new ResizeObserverEntry()]);
        }
        else
        {
            if (!domEventSubscriptionStore.TryGetValue(key, out var value))
            {
                ...
                {
                    for (var i = 0; i < domEventSubscriptionStore[key].Count; i++)
                    {
                        var subscription = domEventSubscriptionStore[key][i];
            ...
            value.Add(new DomEventSubscription(callback, typeof(List<ResizeObserverEntry>), _id));
        }
    }
